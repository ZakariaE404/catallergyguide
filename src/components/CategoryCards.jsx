...
const categories = [
  ...
  },
  {
    ...
    icon: (
      <svg className="w-10 h-10 text-sage" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
        {/* Shopping bag with paw print icon */}
  ...
  {
    title: 'Managing Allergies at Home',
    href: '#',
    icon: (
      <svg className="w-10 h-10 text-sage" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
        {/* House with heart inside icon */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {categories.map((cat, idx) => (
            <Link
              key={idx}
              href={cat.href}
              className="group bg-beige hover:bg-[#E8DFC8] p-8 rounded-2xl flex flex-col items-center justify-center text-center transition-all duration-300 transform hover:-translate-y-1 shadow-sm hover:shadow-md border border-charcoal/5"
            >
              {/* Icon Container */}
